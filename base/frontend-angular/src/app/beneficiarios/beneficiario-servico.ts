...
export class BeneficiarioServico {
  ...
  listar(filtros: FiltrosBeneficiarios): Observable<PaginaBeneficiarios> {
    let params = new HttpParams().set('pagina', filtros.pagina).set('tamanho', filtros.tamanho);
    if (filtros.status) params = params.set('status', filtros.status);
    if (filtros.plano_id) params = params.set('plano_id', filtros.plano_id);
    return this.http.get<PaginaBeneficiarios>(`${this.base}/beneficiarios`, { params });
  }
  criar(dados: CriarBeneficiario): Observable<Beneficiario> {
    return this.http.post<Beneficiario>(`${this.base}/beneficiarios`, dados);
  }
  atualizar(id: string, dados: AtualizarBeneficiario): Observable<Beneficiario> {
